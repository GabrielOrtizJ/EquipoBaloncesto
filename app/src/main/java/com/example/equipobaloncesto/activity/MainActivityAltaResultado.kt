...
class MainActivityAltaResultado : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val puntosVe = findViewById<EditText>(R.id.editTextTextPEVisitante)
    ...
    suspend fun databaseAccess(equipoL:String,equipoV:String,puntosEL:Int,puntosVe:Int,fecha:String) {
        ...
        if (equipoExistenteLocal != null && equipoExistenteVisitante != null) {
            ...
            try {
                ...
                // Actualizar la clasificación del equipo con más puntos
                val equipoGanador =
                    if (puntosEL > puntosVe){
                        equipoL
                    } else{
                        equipoV
                    }
                var clasificacion = clasificacionDao.getClasificacionByEquipo(equipoGanador)
                clasificacion.puntos += 1

                clasificacion.id?.let { clasificacionDao.updateClasamentoById(it, clasificacion.puntos) }
